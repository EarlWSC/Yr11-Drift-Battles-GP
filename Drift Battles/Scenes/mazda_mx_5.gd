...
# First # Value is Steering Range and Second # Value is Speed of steering from left to right/ vice versa
	...
	var acceleration = Input.get_axis("back", "forward") * 800 #Horsepower
...
func apply_handbrake(apply: bool):
	...
	if handbrake_applied:
		...
		$LR_Wheel.wheel_friction_slip = 0.2 # Lower friction slip to allow drifting
		$RR_Wheel.wheel_friction_slip = 0.2
	else:
